...
function App() {
  ...
  useEffect(() => {
    const fallbackImage = typeof window !== 'undefined' ? new URL('/vite.svg', window.location.origin).toString() : ''
    initOpenGraphFromLocation({
      fallbackMeta: {
        title: 'APOD Mood Gallery',
        description: "Explore NASA's Astronomy Picture of the Day through moods, palettes, and collections.",
        image: fallbackImage,
  ...
  useEffect(() => {
    const sync = startBackgroundApodSync({ intervalMs: 6 * 60 * 60 * 1000, maxLookbackDays: 21 })
    return () => {
      sync?.stop?.()
    }
  }, [])
      ...
      <header className="mx-auto max-w-6xl mb-8">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
          ...
          <nav className="flex items-center gap-2 overflow-x-auto pb-2 md:pb-0">
            ...
            <NavButton active={activeView === 'collections'} onClick={() => setActiveView('collections')}>
              Collections
            </NavButton>
            <NavButton active={activeView === 'foryou'} onClick={() => setActiveView('foryou')}>
              For You
            </NavButton>
            <NavButton active={activeView === 'moodboard'} onClick={() => setActiveView('moodboard')}>
              Mood Board
            </NavButton>
            <NavButton active={activeView === 'timeline'} onClick={() => setActiveView('timeline')}>
              Timeline
            </NavButton>
            <NavButton active={activeView === 'exoplanets'} onClick={() => setActiveView('exoplanets')}>
              Exoplanets
            </NavButton>
        ...
        {!hasApiKey && (
          <div className="mt-6 rounded-lg border border-amber-500/20 bg-amber-500/10 p-3 text-xs text-amber-100">
            <strong>Note:</strong> No API key detected. Using demo mode (limited rates). 
            Create a <code className="font-mono">.env</code> file with <code className="font-mono">VITE_NASA_API_KEY</code> for full access.
          </div>
        )}
      </header>
